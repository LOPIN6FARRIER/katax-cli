...
**Options:**
- `-m, --method <method>` - HTTP method (GET, POST, PUT, PATCH, DELETE) (default: `POST`)
...
The single utils file actually generated by `katax init` (there is no separate `response.utils.ts` file):

```typescript
interface ControllerResult<T = any> { success: boolean; message?: string; data?: T; error?: string; statusCode: number; ... }
interface ResponseCookie { ... }
interface ValidationError { ... }
interface ValidationResult<T = unknown> { ... }
interface SendResponseOptions<...> { ... }
interface KataxSchema<T = unknown> { ... }

createSuccessResult<T>(...)
createErrorResult(...)
validateSchema<T>(schema, data)
sendResponse<TValidation, TResponse>(req, res, { validator, controller, dataSource, ... })
setCookie(...)
clearCookie(...)
```
...
ifOk(result, fn), ifErr(result, fn)
```
...
toAppError(error): AppError
```

### JWT Auth Utilities (`auth.utils.ts`, when auth enabled)

- `hashPassword(password, saltRounds?)` - Hash with bcrypt
- `verifyPassword(password, hash)` - Verify a bcrypt hash
- `hashPasswordArgon2(password)` - Hash with argon2
- `verifyPasswordArgon2(password, hash)` - Verify an argon2 hash
- `generateToken(payload, ...)` - Generate JWT access token
- `verifyToken<T>(token, ...)` - Verify and decode a token
- `decodeToken<T>(token)` - Decode without verification (debugging)
- `generateRefreshToken(payload, ...)` - Generate JWT refresh token
- `generateRandomToken(length?)` - Random hex string (API keys, reset tokens)
- `generateNumericCode(digits?)` - Numeric OTP code
- `sha256(data)` / `md5(data)` - Hashing functions
- `encrypt(text, secretKey)` / `decrypt(encryptedData, secretKey)` - AES-256-GCM encryption
