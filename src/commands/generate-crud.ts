...
export async function generateCrudCommand(
  ...
  try {
    ...
    gray(
      `  src/api/${resourceName.toLowerCase()}/${resourceName.toLowerCase()}.validator.ts`,
    );
    gray(
      `  src/api/${resourceName.toLowerCase()}/${resourceName.toLowerCase()}.controller.ts`,
    );
    gray(
      `  src/api/${resourceName.toLowerCase()}/${resourceName.toLowerCase()}.handler.ts`,
    );
    gray(
      `  src/api/${resourceName.toLowerCase()}/${resourceName.toLowerCase()}.routes.ts`,
    );
    ...
    selectedMethods.forEach((method) => {
      if (method === "GET") {
        ...
        gray(
          `  ${method.padEnd(6)} /api/${resourceName.toLowerCase()}/:id - Update`,
        );
