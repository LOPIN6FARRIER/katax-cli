...
export class ProjectStructureGenerator {
  ...
  private async generateResponseUtils(): Promise<void> {
    const { generateResponseUtils } =
      await import("../templates/generators/response-utils-template.js");
...
`;
  ...
  private async generateEnvValidator(): Promise<void> {
    ...
    if (this.config.validation === "katax-core") {
      builder
        ...
        .line(
          `  NODE_ENV: k.enum(['development', 'production', 'test']).optional(),`,
        );
  ...
  private async generateErrorMiddleware(): Promise<void> {
    ...
    builder
      ...
      .line()
      .line(
        "  const error = isAppError(err) ? err : InternalServerError.fromError(err);",
      )
      .line()
  ...
  private async generateDatabaseConnection(): Promise<void> {
    ...
    if (this.config.database === "postgresql") {
      ...
      builder
        ...
        .line(
          "  connectionLimit: Number(process.env.DB_POOL_MAX) || 10, // Max connections",
        )
        .line(
      ...
      builder
        ...
        .line(
          "const uri = `${protocol}://${authString}${host}/${dbName}?retryWrites=true&w=majority`;",
        )
        .line()
        .comment("MongoDB client options - Production settings")
        .line("const options: MongoClientOptions = {")
        .line(
          "  maxPoolSize: Number(process.env.DB_POOL_MAX) || 10,  // Max connections",
        )
        .line(
          "  minPoolSize: Number(process.env.DB_POOL_MIN) || 2,   // Min connections",
        )
        .line(
...
`;
  ...
  private async generateHealthCheck(): Promise<void> {
    ...
    builder.import(["Request", "Response"], "express").line();

    // Import health check if database is configured
    if (this.config.database !== "none") {
      builder
        .import(["checkDatabaseHealth"], "../database/connection.js")
        .line();
    }
    ...
    if (this.config.database !== "none") {
      builder
        ...
        .line("    Object.assign(health, {")
        .line(
          '      database: { status: \"error\", type: \"${this.config.database}\" }',
        )
        .line("    });")
        .line("  }")
        .line()
      ...
      builder.line("  };").line().line("  res.json(health);").line("}");
    }
